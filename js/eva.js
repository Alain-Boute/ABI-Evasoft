...
** -------------------------------------------------------------------
**        File : eva.js
** Description : utility functions for CGI engine
**      Author : Alain BOUTE
**     Created : 2002
*********************************************************************/

// Container used to hide/show items
var comFRAME = null;
...
// Output a button
function ob(href,img,img1,label,tooltip,border,mode,w,h,ctlclc)
{
	if(href != imgaction) imgnum++;
	imgaction = href;
	if(!ctlclc) ctlclc = href;
	var num = imgnum;
	...
	var pos = ctlclc.indexOf('#')
	if(pos > 3) document.write(' ctrlclic="I'+ctlclc.substr(1, pos)+'"');
	document.write('>');
	if(img != '')
	{
...
function ShowHelp(elt)
{
	...
	if(helpmsg && helpmsg != '' && (elt.getAttribute("helpmsg") == "" || elt.getAttribute("helpmsg") == null))
	{
...
function ShowpHelp(e)
{
    ...
    // Hide block when mouse out
    elt.onmouseout = function(){ hideDIV(); HideHelp();}
	p.onmouseout = function(){ hideDIV(); HideHelp();};

	// Handle open control when clic on help text
	var ctrlclic  = elt.getAttribute("ctrlclic");
	if(!ctrlclic || ctrlclic == '')
	{
		p.onclick = function(){ hideDIV(); HideHelp();};
	}
	else
	{
		p.onmouseover = function(){ showDIV(pHelp);};
		p.onclick = function(){ cb(ctrlclic);};
	}
